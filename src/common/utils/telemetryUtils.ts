...
function anonymizeFilePathsInLine(line: string, cleanupPatterns: RegExp[]): string {
    ...
    try {
      ...
      while ((match = pattern.exec(line)) !== null) {
        ...
        if (++iterationCount > maxIterations) {
          console.warn(
            `Warning: Cleanup pattern ${regexp.source} exceeded ${maxIterations} iterations, breaking to prevent infinite loop`,
          );
          patternFailed = true;
          break;
        }
        ...
        if (iterationCount > 1 && match.index === patternMatches[patternMatches.length - 1]?.[0]) {
          console.warn(
            `Warning: Cleanup pattern ${regexp.source} is stuck at position ${match.index}, breaking to prevent infinite loop`,
          );
          patternFailed = true;
          break;
        }
  ...
  while (true) {
    ...
    if (++filePathIterationCount > maxFilePathIterations) {
      console.warn(
        `Warning: File path regex exceeded ${maxFilePathIterations} iterations, breaking to prevent infinite loop`,
      );
      break;
    }
